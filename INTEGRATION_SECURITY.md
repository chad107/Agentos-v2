...
2. **A separate, tenant-scoped, encrypted token store.** Two viable
   ...
   - A dedicated `integration_credentials` table — added and verified
     against a real Postgres 16 instance in Phase 3A
     (`db/migrations/0006_integration_credentials.sql`,
     `DATABASE_DESIGN.md` "Integration credentials"). The table stores
     opaque ciphertext (`BYTEA`) plus an `encryption_key_id` reference; it
     has no dependency on, and no opinion about, which KMS the ciphertext
     is actually encrypted with (AWS KMS, GCP KMS, HashiCorp Vault) — the
     database itself should never hold the decryption key, and no code
     encrypts/decrypts against this table yet, since that depends on the
     KMS choice below.
...
| OAuth/token-storage architecture | Table designed and verified against real Postgres (Phase 3A, `0006_integration_credentials.sql`); encryption/KMS integration and real OAuth flows not implemented |
